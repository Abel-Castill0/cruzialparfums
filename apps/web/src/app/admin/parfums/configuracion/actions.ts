...
export async function updateParfumsBusinessLegalSettingAction(
  expectedUpdatedAt: string,
  _previous: BusinessLegalSettingActionState,
  formData: FormData,
  ...
  const validation = validateBusinessLegalSettingForm(entries(formData));
  if (!validation.ok) return { status: "field_errors", errors: validation.errors };
  ...
  const result = await repository.updateBusinessLegal(expectedUpdatedAt, validation.value);
  if (!result.ok) {
    switch (result.error.type) {
      case "unauthorized":
        return { status: "error", message: "Tu sesión expiró. Vuelve a iniciar sesión." };
  ...
  revalidatePath("/admin/parfums/configuracion");
  revalidatePath("/parfums/privacidad");
  revalidatePath("/parfums/terminos");
  revalidateParfumsStorefront();
  return { status: "success", data: result.data };
}
