...
const EXPECTED_PUBLIC_ID = "cruzial/parfums/products/11111111-1111-4111-8111-111111111111/authorized-asset";
...
describe("registerMediaAction — destructive-cleanup replay regression", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("never destroys a Cloudinary asset when a valid token is replayed with an invalid upload result", async () => {
    // Simulates the exact ChatGPT-reported scenario: the same authorization
    // token is still cryptographically valid within its TTL (it is not
    // single-use), but this particular request's uploadResult doesn't match
    // what was authorized — e.g. a stale/mismatched replay after the
    // authorized asset was already uploaded and registered once.
    ...
    const result = await registerMediaAction(
      PRODUCT_ID,
      null,
      uploadResult({ publicId: "cruzial/parfums/products/11111111-1111-4111-8111-111111111111/tampered" }),
      null,
      false,
      "still-valid-token",
    );
