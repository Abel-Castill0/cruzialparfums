...
const STATUS_LABELS: Record<string, string> = {
  confirmed: "Confirmar por WhatsApp",
  fulfilled: "Marcar atendida",
...
export function OrderStatusControls({ orderId, currentStatus, allowedTransitions }: Props) {
  ...
  const [state, formAction, isPending] = useActionState(
    async (_prev: Awaited<ReturnType<typeof updateParfumsOrderStatusAction>> | null, formData: FormData) => {
      const newStatus = formData.get("new_status") as string;
      const reason = formData.get("reason") as string | null;
      const result = await updateParfumsOrderStatusAction(orderId, currentStatus, newStatus, reason || undefined);
