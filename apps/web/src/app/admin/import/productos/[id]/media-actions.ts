...
function productEditPath(productId: string): Route {
  return `/admin/import/productos/${productId}` as Route;
}
...
function friendlyError(error: AdminRepositoryError): string {
  switch (error.type) {
    ...
    case "unknown":
      console.error("[admin-import:media] unexpected repository error:", error.message);
...
export async function getUploadAuthorizationAction(
  ...
  const authorization = createUploadAuthorization(productId, "import");
  if (!authorization) {
