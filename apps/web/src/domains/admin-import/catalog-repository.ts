...
export class AdminImportCatalogRepository {
  ...
  async updateProduct(id:string,expected:string,input:{name:string;brand:string|null;categoryId:string|null;publicationStatus:string}){const {data,error}=await this.rpc("admin_update_import_product",{p_product_id:id,p_expected_updated_at:expected,p_name:input.name,p_brand:input.brand,p_category_id:input.categoryId,p_publication_status:input.publicationStatus});return error?{ok:false as const,error:mapPostgrestError(error)}:{ok:true as const,data:data as ProductRow};}
