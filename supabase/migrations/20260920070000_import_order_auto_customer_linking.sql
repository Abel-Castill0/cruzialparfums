...
--
-- Automatic Import customer linking.
--
-- create_import_order_request already resolves an EXACTLY-ONE phone match
-- to that customer (returning status only when actually returning) and
-- fails closed (no link) on an AMBIGUOUS match (>1). This migration adds
-- the missing ZERO-match case: today it silently leaves the order
-- unlinked, requiring the admin's manual "Registrar y vincular cliente"
-- step for every genuinely new customer. It now atomically creates a
-- `pending_verification` customer and links it — the order still uses
-- new-customer deposit semantics (50%), and the customer is never
-- auto-promoted to returning; that stays a manual, later decision.
--
-- Concurrency: reuses the EXISTING uniqueness protection —
-- customers_import_active_phone_uniq, a unique index on
...
-- =========================================================================
-- create_import_order_request — auto-link a genuinely new customer
-- instead of leaving the order unlinked
-- =========================================================================
...
begin
  ...
  -- Resolve current Import campaign with FOR UPDATE lock
  ...
  -- Resolve customer by normalized phone — COUNT-based.
  -- EXACTLY ONE non-archived customer -> link it (returning / 70% only when
  --   the linked customer's own status is actually 'returning').
  -- ZERO matches -> atomically create a pending_verification customer and
  --   link it, still under new-customer (50%) deposit semantics. Never
  --   auto-promoted to returning — that stays a manual decision.
  -- AMBIGUOUS (>1) -> fail closed: no link, new-customer semantics. Never
  --   arbitrarily choose among matches.
  ...
  -- Resolve deposit policy — exact time-window + exactly-one
  with applicable as (
    select dp.*
