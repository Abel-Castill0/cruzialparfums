-- Gate B concurrency: deterministic inventory lock order for Parfums orders.
--
-- Each order line's reservation trigger (app.reserve_tracked_order_line)
-- locks that variant's inventory row FOR UPDATE, and
-- create_parfums_order_request_v2 inserted lines in client-supplied order.
-- Two concurrent orders with lines {A,B} and {B,A} could therefore lock in
-- opposite orders and deadlock, failing one customer's order (reproduced in
-- supabase/tests/52_gate_b_order_inventory_lock_order.sql).
--
-- The function body is unchanged except for one statement: before any line is
-- inserted, all inventory rows the order references are locked in
-- inventory.id order, the same order the release/consume path
-- (app.apply_order_inventory_transition) already uses. CREATE OR REPLACE
