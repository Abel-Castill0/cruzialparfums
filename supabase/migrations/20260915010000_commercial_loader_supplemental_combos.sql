-- 4K-C1B: controlled commercial loader supplemental identity and variant-aware combos.
--
-- Source product identity is explicit: non-null legacy_id is authoritative;
-- explicit null legacy_id selects the canonical Parfums slug. Variants use the
-- reconciliation identity (variant_kind, size_ml), never their display label.

create or replace function app.commercial_product_identity(p_product jsonb)
returns text
language sql
immutable
set search_path = ''
as $$
  select case
    when not (p_product ? 'legacy_id') then null
    when p_product->>'legacy_id' is not null and p_product->>'legacy_id' <> ''
      then 'legacy:' || (p_product->>'legacy_id')
    when p_product->'legacy_id' = 'null'::jsonb
         and coalesce(p_product #>> '{target_product,slug}', '') <> ''
      then 'supplemental:' || (p_product #>> '{target_product,slug}')
    else null
  end
$$;

create or replace function app.commercial_reference_identity(p_reference jsonb)
returns text
language sql
immutable
set search_path = ''
as $$
  select case
    when not (p_reference ? 'legacy_id') then null
    when p_reference->>'legacy_id' is not null and p_reference->>'legacy_id' <> ''
      then 'legacy:' || (p_reference->>'legacy_id')
    when p_reference->'legacy_id' = 'null'::jsonb
         and coalesce(p_reference->>'slug', '') <> ''
      then 'supplemental:' || (p_reference->>'slug')
    else null
  end
$$;

create or replace function app.commercial_variant_identity(p_variant jsonb)
returns text
language sql
immutable
set search_path = ''
as $$
  select case
    when coalesce(p_variant->>'variant_kind', '') = '' or not (p_variant ? 'size_ml') then null
    else (p_variant->>'variant_kind') || ':' ||
      case when p_variant->'size_ml' = 'null'::jsonb then 'null'
           else ((p_variant->>'size_ml')::numeric)::text end
  end
$$;
...
begin
  ...
  for p in select value from jsonb_array_elements(coalesce(p_manifest->'products','[]'::jsonb))
    ...
    if product_row.id is not null and product_ok then
      for variant_row in select x.* from public.product_variants x where x.product_id=product_row.id and not exists (
        ...
        where x.variant_kind=s->>'variant_kind' and x.size_ml is not distinct from (s->>'size_ml')::numeric) loop
        variants_conflict:=variants_conflict+1;
  ...
  return jsonb_build_object('mode','dry_run','applied',false,'refused',false,
    'operations',jsonb_build_object(
      'categories',jsonb_build_object('insert',categories_insert,'unchanged',categories_unchanged,'conflict',categories_conflict),
      'products',jsonb_build_object('insert',products_insert,'unchanged',products_unchanged,'conflict',products_conflict),
      'variants',jsonb_build_object('insert',variants_insert,'unchanged',variants_unchanged,'conflict',variants_conflict),
      'relationships',jsonb_build_object('insert',relationships_insert,'unchanged',relationships_unchanged,'conflict',relationships_conflict),
      'inventory',jsonb_build_object('insert',inventory_insert,'unchanged',inventory_unchanged,'conflict',inventory_conflict),
      'combos',jsonb_build_object('insert',combos_insert,'unchanged',combos_unchanged,'conflict',combos_conflict),
      'combo_items',jsonb_build_object('insert',items_insert,'unchanged',items_unchanged,'conflict',items_conflict)),
