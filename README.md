...
8. **Fotos de producto sin fondo**: `img/perfumes/transparent/*.webp` son las 194 fotos del catálogo con el fondo de estudio quitado (`.claude/scripts/batch_remove_bg.py`, mismo flood-fill por gradiente que el hero — sin IA externa). `assets/data.js` (`IMG_MAP`) apunta a estas por defecto. **14 productos** con botella de vidrio transparente (el fondo gris y el vidrio tienen luminancia casi idéntica, así que el flood-fill se come el cuerpo de la botella) se quedan a propósito en su PNG original de `img/perfumes/` — ver `bg_removal_exclude.json` para la lista exacta. Si agregas un producto nuevo: corré `batch_remove_bg.py`, revisá el resultado contra un fondo de color antes de usarlo (`bg_removal_report.json` marca `"flag":"review"` cuando la proporción de fondo detectada es implausible, pero no detecta todos los casos de vidrio transparente — mirarlo es la única garantía real), y si falla, dejá el PNG original apuntado en `IMG_MAP` en vez de forzar el recorte.
